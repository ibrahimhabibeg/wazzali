...
const EditMyDataModal = ({ visible, hide }: propsType): React.JSX.Element => {
  ...
  const handleUsernameChange = (newUsername: string): void => {
    ...
    if (newUsername !== me?.username && ((users?.some((user) => user.username === newUsername)) ?? false)) { setUsernameError('Username already taken.') }
  }

  const handleDescriptionChange = (newDescription: string): void => { setDescription(newDescription) }
